...
Clone the plugin repo temporarily and seed skills into Fenix using the MCP tools (same as OpenCode Step 5). For each skill in `seed/seed-skills.json`, use `skill_create` or `skill_update` via the Fenix MCP.
